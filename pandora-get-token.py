...
users_file = os.path.join(current_work_dir, "users.txt")
session_tokens_file = os.path.join(current_work_dir, "session_tokens.txt")
tokens_file = os.path.join(current_work_dir, "tokens.txt")
...
def process_users(users_file, session_tokens_file):
    ...
    print_log(
        f"Successfully extracted {len(session_tokens)} session tokens, Saved to file {session_tokens_file}"
    )
